...
bool Song::matches_keyword(const string &kw) const {
    ...
    if (to_lower_copy(title_).find(low_k) != string::npos) return true;
    if (to_lower_copy(artist_).find(low_k) != string::npos) return true;
    for (const auto &t : tags_) if (to_lower_copy(t).find(low_k) != string::npos) return true;
